...
window.addEventListener('load', async () => {
  ...
  if (Notification.permission === 'denied') return; // Leave him or her alone!
    ...
    setTimeout(async () => { // Delay notifications to allow the guy to at least see the site's content
      alert("💡 Pour recevoir nos dernières nouvelles, veuillez autoriser les notifications sur ce site !");
...
async function getCurrentUser() {
  let url = window.location.host.includes("localhost") ? "http://localhost:5000" : "https://nextgenbuilders.onrender.com";
